%% Almost all the codes in this function are borrowed from Matlab built-in
%% function estimateGeometricTransform.m 
%% Copyright 2014, Matlab

function [tform, inliers,status] ...
    = affinematrixestimation(matchedPoints1, matchedPoints2, ...
...
%% Adding: return to inliers index
...
if status == statusCode.NoError
    ...
    % Do an extra check to verify the tform matrix
    if isequal(det(tmatrix),0) % check if matrix is singular
        status = statusCode.NotEnoughInliers;
